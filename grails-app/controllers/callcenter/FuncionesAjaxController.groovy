...
class FuncionesAjaxController {
    ...
    def calucularEasyCode(){
        boolean visible = false
        if(params.nombreBase != ""){
            visible = true
            def nombreBase = params.list("nombreBase")
            def condiciones = [nombreBase: nombreBase]
            String sql = "select cast(codigoAsignacion as integer) from Clientes where nombreBase in (:nombreBase) group by cast(codigoAsignacion as integer)"
            int dato1 = 0
            def codigo = Clientes.executeQuery(sql, condiciones)
            for(int i = 0; i < codigo.size(); i++){
                Clientes dato = codigo.get(i)
                dato1 = dato.codigoAsignacion.toInteger()
            }
            //render codigo as JSON
            println(dato1)
        }
    }
    ...
    def calcularDisponiblesAsignar(){
        ...
        if(tipoRegistros.equalsIgnoreCase("SIN GESTION")){
            ...
            if(tipoRegistros.equalsIgnoreCase("RELLAMADAS")){
                ...
                if(subestado != ""){
                    condicionSubestado = " and subestadoGestion.nombre = :subestado"
    ...
    def calcularDisponiblesAsignarNovedades(){
        ...
        if(tipoRegistros.equalsIgnoreCase("SIN GESTION")){
            ...
            sql = "from ClientesNovedades where subestadoGestion.type = 'Regestionable' and usuario.rol != :rol and nombreBase = :nombreBase and intentos != 0 ${condicionEasyDesde} ${condicionEasyHasta} ${condicionIntentosDesde} ${condicionIntentosHasta} ${condicionSubestado}"
        }
    ...
    def getSubSubStatusBySubStatus(){
        ...
        def array = [subSubStatus.name, subSubStatus.name, subStatus.type, subStatus.enableManagement, subStatus.enableManagement2]
        render array as JSON
    }
    ...
    def getParroquias() {
        ...
        if (params.id) {
            ...
            def array = [parroquias.nombre, parroquias.nombre]
            render array as JSON
        }
    }
    ...
    def getPlanes() {
        ...
        if (params.id) {
            def producto = params.id
            def tipos = Producto.executeQuery(" from Producto where nombre = :producto order by marca", [producto: producto])
            def array = [tipos.marca, tipos.marca]
            render array as JSON
        }
    }
    ...
    def getGraficoVentasHora() {
        ...
        def contactadosPorHora = Clientes.executeQuery("select cast(substring(fechaGestion, 12, 2) as int) as dia, count(*) as cantidad from Clientes where estadoGestion = 'CONTACTOS EFECTIVOS' and fechaGestion between :fechaInicio and :fechaFin group by cast(substring(fechaGestion, 12, 2) as int) order by dia", [fechaInicio: fechaInicio, fechaFin: fechaFin])
    ...
    def getGrafContVsNocont() {
        ...
        def detalleContactados = Clientes.executeQuery("select subestadoGestion.nombre, count(*) from Clientes where fechaGestion between :fechaInicio and :fechaFin and estadoGestion = 'CONTACTOS EFECTIVOS' group by subestadoGestion.nombre", [fechaInicio: fechaInicio, fechaFin: fechaFin])
        def detalleNoContactados = Clientes.executeQuery("select subestadoGestion.nombre, count(*) from Clientes where fechaGestion between :fechaInicio and :fechaFin and estadoGestion = 'NO CONTACTO' group by subestadoGestion.nombre", [fechaInicio: fechaInicio, fechaFin: fechaFin])

        for (int i = 0; i < consulta.size; i++) {
            if (consulta[i][0] == 'CONTACTOS EFECTIVOS') {
    ...
    def getGraficoVentasHoraReporte() {
        ...
        String sqlContactados = "select cast(substring(fechaGestion, 12, 2) as int) as dia, count(*) as cantidad from Clientes where estadoGestion = 'CONTACTOS EFECTIVOS' and fechaGestion between :fechaInicio and :fechaFin ${condicionUsuario} ${condicionNombreBase} group by cast(substring(fechaGestion, 12, 2) as int) order by dia";
    ...
    def getGraficoEstadosReporte() {
        ...
        String sqlDetalleContactados = "select subestadoGestion.nombre, count(*) from Clientes where fechaGestion between :fechaInicio and :fechaFin and estadoGestion = 'CONTACTOS EFECTIVOS' ${condicionUsuario} ${condicionNombreBase} group by subestadoGestion.nombre";
        String sqlDetalleNoContactados = "select subestadoGestion.nombre, count(*) from Clientes where fechaGestion between :fechaInicio and :fechaFin and estadoGestion = 'NO CONTACTO' ${condicionUsuario} ${condicionNombreBase} group by subestadoGestion.nombre";
    ...
    def rellamadas() {
        ...
        /*if (request.xhr) {
            Usuario usuario = Usuario.findById(session.user.id) //Obtiene el usuario
        ...
        if(request.xhr){
            ...
            String consulta = "FROM Clientes WHERE subestadoGestion.type = 'Rellamada' AND fechaRellamada between :fechaInicioDia and :fecha AND usuario = :usuario and agendamientoAsesor = 'AGENDAR PARA MI' order by fechaRellamada desc"
            def condiciones = ["usuario": usuario, fecha: new Date(), fechaInicioDia: Date.parse("yyyy-MM-dd HH:mm:ss", new Date().format("yyyy-MM-dd 00:00:00"))];
            def rellamadas = Clientes.executeQuery(consulta, condiciones);
            render rellamadas as JSON
        }
    ...
    def GuardarReferido(){
        ...
        cliente.fechaCaducidad = "2030-12-31"
    ...
    def getValoresPlan() {
        ...
        if (params.id) {
            def plan = params.id
            def producto = params.idProducto
            def tipos = Producto.executeQuery("select valor from Producto where nombre = :producto and marca = :plan order by marca", [producto: producto, plan: plan])
            /*def array = [tipos.marca, tipos.marca]
            render array as JSON*/
            ///println tipos + " PPP"
            render tipos as JSON
