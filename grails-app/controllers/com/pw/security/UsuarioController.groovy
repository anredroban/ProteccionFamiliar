...
class UsuarioController {
	...
	def login(){
		...
		if(session.user){
			//session.user = usuario
			if(Util.isOperator(session.user.usuario)){
				redirect(action:'dashboardAgente')
			}else {
				redirect(action:'dashboard')
			}
		...
		if(params.usuario && params.password){
			...
			if(usuario){
				...
				if(!sesion.save(flush: true))
					println sesion.errors
				if(Util.isOperator(usuario.usuario)){
					redirect(action:'dashboardAgente')
				}else {
					redirect(action:'dashboard')
				}
	...
	def dashboard(){
		...
		def subestados = Subestado.executeQuery("from Subestado where enableManagement = true")
		Rol rol = Rol.findByNombre("OPERADOR")
		def ventasPorUsuario = Clientes.executeQuery("select nombreVendedor, count(*) from Clientes where subestadoGestion in (:subestados) and fechaGestion between :fechaInicio and :fechaFin group by nombreVendedor order by nombreVendedor", [subestados: subestados, fechaInicio: fechaInicio, fechaFin: fechaFin])
...
//		String[][] consolidado = consolidarExitAdic(ventasPorUsuario, adicionalesPorUsuario)
	...
	//	def productosVendidos = Adicional.executeQuery("select b.marca, count(*) from Adicional a, Producto b where a.productoAceptado = b.nombre and a.clientes.fechaGestion between :fechaInicio and :fechaFin and a.clientes.subestadoGestion = 12 group by b.marca", [fechaInicio: fechaInicio, fechaFin: fechaFin])
...
//		int totalNoContactados = 0
		...
		def contactadosAgente = Historial.executeQuery("select nombreVendedor, count(*) from Historial where intentos != 0 and estadoGestion = 'CONTACTOS EFECTIVOS' and fechaGestion between :fechaInicio and :fechaFin group by nombreVendedor order by nombreVendedor", [fechaInicio: fechaInicio, fechaFin: fechaFin])
	//	def ventasPorHoraVendedor = Clientes.executeQuery("select hour(a.fechaGestion), a.nombreVendedor, count(b.id) from Clientes a , Adicional as b where a.subestadoGestion in (:subestados) and a.fechaGestion between :fechaInicio and :fechaFin and a.id = b.clientes.id group by hour(a.fechaGestion), a.nombreVendedor", [subestados: subestados, fechaInicio: fechaInicio, fechaFin: fechaFin])
	//	def encuestasVehicular  = Clientes.executeQuery("select nombreVendedor, count(*) from Clientes where tipoProducto = 'APLICA CREDITO VEHICULAR' and ( aceptacionCreditoVehicular is not null and aceptacionCreditoVehicular != '' ) and fechaGestion between :fechaInicio and :fechaFin group by nombreVendedor order by nombreVendedor", [fechaInicio: fechaInicio, fechaFin: fechaFin])
//		def noContactadosAgente = Clientes.executeQuery("select nombreVendedor, count(*) from Historial where intentos != 0 and estadoGestion = 'NO CONTACTADO' and fechaGestion between :fechaInicio and :fechaFin group by nombreVendedor order by nombreVendedor", [fechaInicio: fechaInicio, fechaFin: fechaFin])
		String[][] tablaResult = new String[gestionadosAgente.size()][9]
        String [][] ventasFinal = new String [2]
		...
		for(int i = 0; i < tablaResult.size(); i++){
			...
			for(int j = 0; j < ventasPorUsuario.size(); j++){
				if(ventasPorUsuario[j][0] == gestionadosAgente[i][0]){
                    ...
                    ventasFinal[0] = ventasPorUsuario[j][1]
					break
				}
			}
			...
			for(int j = 0; j < adicionalesPorUsuario.size(); j++){
				if(adicionalesPorUsuario[j][0] == gestionadosAgente[i][0]){
					...
					tablaResult[i][7] = df.format((adicionalesPorUsuario[j][1] / ventasPorUsuario[j][1])).replace(",", ".")
					break
				}
			}
			//Lleno la informacionde la encuesta
			/*for(int k = 0; k < encuestasVehicular.size(); k++){
				if(encuestasVehicular[k][0] == gestionadosAgente[i][0]){
					tablaResult[i][8] = encuestasVehicular[k][1]
					break
				}
			}*/
		}
		...
		if(totalGestionados == 0){
			...
			if(totalExitosos != 0) {
				totalColocacionPorCliente = df.format(totalAdicionales / totalExitosos)
			}else{
				totalColocacionPorCliente = "0.00"
			}
		...
		[inicioSesion: inicioSesion, tablaResult: tablaResult, totalGestionados: totalGestionados, totalContactados: totalContactados, totalExitosos: totalExitosos, /*tablaResult1: tablaResult1, totalPorcentajeContactabilidad: totalPorcentajeContactabilidad,*/ totalAdicionales: totalAdicionales, totalPorcentajeEfectividad: totalPorcentajeEfectividad, totalColocacionPorCliente: totalColocacionPorCliente /*ventasPorHoraVendedor:ventasPorHoraVendedor, productosVendidos: productosVendidos*/
		 ,tableTiemposAgentes: tableTiemposAgentes, totalEfectivosGrupo: totalEfectivosGrupo, totalMetaGrupo: totalMetaGrupo, totalFaltantesGrupo: totalFaltantesGrupo,
		  totalPorcentajeMetaGrupo: totalPorcentajeMetaGrupo, totalPromedioHoraGrupo: totalPromedioHoraGrupo, fechaHoraActualizacion: fechaHoraActualizacion, tablaResult1: tablaResult1]
	...
	def dashboardAgente(){
		...
		[inicioSesion: inicioSesion, tablaResult: tablaResult, totalGestionados: totalGestionados, totalContactados: totalContactados, totalExitosos: totalExitosos, /*tablaResult1: tablaResult1, totalPorcentajeContactabilidad: totalPorcentajeContactabilidad,*/ totalAdicionales: totalAdicionales, totalPorcentajeEfectividad: totalPorcentajeEfectividad, totalColocacionPorCliente: totalColocacionPorCliente/*, ventasPorHoraVendedor:ventasPorHoraVendedor, productosVendidos: productosVendidos*/
		 ,tableTiemposAgentes: tableTiemposAgentes, totalEfectivosGrupo: totalEfectivosGrupo, totalMetaGrupo: totalMetaGrupo, totalFaltantesGrupo: totalFaltantesGrupo,
		 totalPorcentajeMetaGrupo: totalPorcentajeMetaGrupo, totalPromedioHoraGrupo: totalPromedioHoraGrupo, fechaHoraActualizacion: fechaHoraActualizacion/*, totalEncuestas: totalEncuestas*/]
	}
