$(document).ready(function(){
    ...
    $("#calcularLnk").click(function(e){
        ...
        $easyDesde = $("#desde").val().trim();
        $easyHasta = $("#hasta").val().trim();
        $intentosDesde = $("#intentosDesde").val().trim();
        $intentosHasta = $("#intentosHasta").val().trim();
        $.post(baseUrl + "/funcionesAjax/calcularDisponiblesAsignarNovedades", {nombreBase: $nombreBase, tipoRegistros: $tipoRegistros, subestado: $subestado, easyDesde: $easyDesde
                , easyHasta: $easyHasta  , intentosDesde: $intentosDesde, intentosHasta: $intentosHasta}, function(data){
