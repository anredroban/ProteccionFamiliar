...
public class Util {
	...
	static int getCantidadVentasMes(){
		Calendar cal = Calendar.getInstance();
		cal.set(Calendar.DAY_OF_MONTH, cal.getActualMinimum(Calendar.DAY_OF_MONTH));
		Date fechaInicio = Date.parse("yyyy-MM-dd HH:mm:ss", cal.getTime().format("yyyy-MM-dd 00:00:00"))
		Date fechaFin = Date.parse("yyyy-MM-dd HH:mm:ss", new Date().format("yyyy-MM-dd 23:59:59"))
	...
	static String getContactabilidadMensual(){
		...
		Date fechaInicio = Date.parse("yyyy-MM-dd HH:mm:ss", cal.getTime().format("yyyy-MM-dd 00:00:00"))
		Date fechaFin = Date.parse("yyyy-MM-dd HH:mm:ss", new Date().format("yyyy-MM-dd 23:59:59"))
		int contactados = Clientes.executeQuery("select count(*) from Clientes where estadoGestion = 'CONTACTOS EFECTIVOS' and fechaGestion between :fechaInicio and :fechaFin", [fechaInicio: fechaInicio, fechaFin: fechaFin]).get(0)
	...
	static String getContactabilidadDiaria(){
		...
		Date fechaInicio = Date.parse("yyyy-MM-dd HH:mm:ss", new Date().format("yyyy-MM-dd 00:00:00"))
		Date fechaFin = Date.parse("yyyy-MM-dd HH:mm:ss", new Date().format("yyyy-MM-dd 23:59:59"))
		int contactados = Historial.executeQuery("select count(*) from Historial where estadoGestion = 'CONTACTOS EFECTIVOS' and fechaGestion between :fechaInicio and :fechaFin", [fechaInicio: fechaInicio, fechaFin: fechaFin]).get(0)
	...
	static int[] getCantidadAsistencias(){
		...
		Date fechaInicio = Date.parse("yyyy-MM-dd HH:mm:ss", new Date().format("yyyy-MM-dd 00:00:00"))
		Date fechaFin = Date.parse("yyyy-MM-dd HH:mm:ss", new Date().format("yyyy-MM-dd 23:59:59"))
		def subestadosEfectivos = Subestado.executeQuery("from Subestado where id in (1,2,3)")
		def ventas = Clientes.executeQuery("select a.nombreVendedor, seguroDesgravamen from Clientes a where a.subestadoGestion in (:subestadosEfectivos) and a.fechaGestion between :fechaInicio and :fechaFin", [subestadosEfectivos: subestadosEfectivos, fechaInicio: fechaInicio, fechaFin: fechaFin])
		for(int i = 0; i < ventas.size(); i++){
			if(ventas[i][1] == "SI")
				cantidadAsistencia++
		}
		salida[0] = cantidadAsistencia
		return salida
	}
	...
	static int getContactados(){
		...
		Date fechaInicio = Date.parse("yyyy-MM-dd HH:mm:ss", new Date().format("yyyy-MM-dd 00:00:00"))
		Date fechaFin = Date.parse("yyyy-MM-dd HH:mm:ss", new Date().format("yyyy-MM-dd 23:59:59"))
		int contactadosQuito = Clientes.executeQuery("from Clientes where estadoGestion = :estado and fechaGestion between :fechaInicio and :fechaFin", [estado: "CONTACTOS EFECTIVOS", fechaInicio: fechaInicio, fechaFin: fechaFin]).size()
	...
	public static ArrayList<String> getProductos(long idCliente){
		...
		productos.add(cliente.productoDiners)
		productos.add(cliente.productoVisa)
		productos.add("KIDS")
		productos.add("FREEDOM")
		return productos
	}
